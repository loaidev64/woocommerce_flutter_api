import '../../helpers/fake_helper.dart';
import '../../json/woo_json.dart';
import 'order.dart';

class WooOrderBatchResponse {
  WooOrderBatchResponse({
    this.create,
    this.update,
    this.delete,
  });
  factory WooOrderBatchResponse.fromJson(Map<String, dynamic> json) =>
      WooOrderBatchResponse(
        create: WooJson.readList(json, 'create', WooOrder.fromJson),
        update: WooJson.readList(json, 'update', WooOrder.fromJson),
        delete: WooJson.readList(json, 'delete', WooOrder.fromJson),
      );
  factory WooOrderBatchResponse.fake() => WooOrderBatchResponse(
        create: FakeHelper.list(() => WooOrder.fake()),
        update: FakeHelper.list(() => WooOrder.fake()),
        delete: FakeHelper.list(() => WooOrder.fake()),
      );
  final List<WooOrder>? create;
  final List<WooOrder>? update;
  final List<WooOrder>? delete;
  Map<String, dynamic> toJson() => <String, dynamic>{}
    ..putIfPresent('create', create?.map((order) => order.toJson()).toList())
    ..putIfPresent('update', update?.map((order) => order.toJson()).toList())
    ..putIfPresent('delete', delete?.map((order) => order.toJson()).toList());
  WooOrderBatchResponse copyWith({
    List<WooOrder>? create,
    List<WooOrder>? update,
    List<WooOrder>? delete,
  }) =>
      WooOrderBatchResponse(
        create: create ?? this.create,
        update: update ?? this.update,
        delete: delete ?? this.delete,
      );
  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is WooOrderBatchResponse &&
          WooJson.listEquals(other.create, create) &&
          WooJson.listEquals(other.update, update) &&
          WooJson.listEquals(other.delete, delete);
  @override
  int get hashCode => Object.hashAll([...?create, ...?update, ...?delete]);
  @override
  String toString() {
    return 'WooOrderBatchResponse(create: ${create?.length ?? 0}, update: ${update?.length ?? 0}, delete: ${delete?.length ?? 0})';
  }
}
